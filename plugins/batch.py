...
@Client.on_message(filters.command('Index') & filters.chat(ADMINS))
async def batch(c,m):
    if len(m.command) < 2:
        ...
        return await m.reply(text=f"Are you sure you want to Add Movies?\n\nChannel: {channel_id}",
...
async def cancel(c:Client, m):
    if m.data == "cancel":
        ...
        info_text = "Adding Movies/Series Started!\n\n Channel: {}\n\nTo Cancel /cancel\n\n Message Saved: {}"
...
async def stop_button(c, m):
    if m.from_user.id in ADMINS:
        ...
        await msg.edit("Adding Movies Stopped Successfully 👍")
